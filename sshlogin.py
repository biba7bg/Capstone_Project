...
def get_credentials():  # creating ceredentials function
    username = "bilayanabily" #input("Enter your username:"4
...
def execute_commands(ssh):  # defining menu and choices function
    while True:
        ...
        if choice == "1":
            stdin, stdout, stderr = ssh.exec_command("systemctl --type=service")
            print(stdout.read().decode())
        elif choice == "2":
            stdin, stdout, stderr = ssh.exec_command("uptime && last reboot | head -n 5")
            print(stdout.read().decode())
        elif choice == "3":
            print("not ready yet")#stdin, stdout, stderr = ssh.exec_command("pgrep -f 'jvm_name'")
            # old_pid = stdout.read().decode()strip()
            # print(f"old JVM OID: {old_pid}")
        elif choice == "4":
            confirm = input("Are you sure you want to restart the server? (yes/no): ")
